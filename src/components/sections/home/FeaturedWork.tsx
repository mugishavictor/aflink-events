...
const featuredCase = {
  ...
  scope: [
    ...
    "Technical setup + show calling",
    "Onsite execution",
  ],
  results: [
    "On-time show delivery",
    "Clean transitions + rehearsed run-of-show",
        ...
        <div className="mx-auto max-w-[1440px] px-5 sm:px-8 py-16 lg:py-20">
              ...
              <div className="rounded-3xl border border-black/5 bg-white/70 p-7 backdrop-blur shadow-[0_12px_40px_rgba(10,16,32,.08)]">
                ...
                <div className="mt-6 space-y-5">
                  <div>
                    ...
                    <div className="mt-1 text-sm leading-relaxed text-[rgb(var(--fg))]/65">
                      {featuredCase.objective}
                    </div>
                  </div>
                      ...
                      {featuredCase.scope.map((s) => (
                        <li key={s} className="flex gap-2">
                          <span
                            ...
                            style={{ background: "rgba(22,92,255,.60)" }}
                          />
                          <span>{s}</span>
                        </li>
                      ))}
                    ...
                    <ul className="mt-2 space-y-2 text-sm text-[rgb(var(--fg))]/65">
                      {featuredCase.results.map((r) => (
                        <li key={r} className="flex gap-2">
                          <span
                            className="mt-[7px] h-1.5 w-1.5 rounded-full"
                            style={{ background: "rgba(0,210,255,.60)" }}
                          />
                          <span>{r}</span>
                        </li>
                      ))}
                ...
                <div className="mt-8 flex gap-3">
                  <Button href={`/work/${featuredCase.slug}`} variant="secondary">
                    View details
                  </Button>
          ...
          <div className="mt-14">
            <div className="flex items-end justify-between gap-6">
              ...
              <Link
                href="/work"
                className="hidden sm:inline text-sm font-medium text-[rgb(var(--fg))]/70 hover:text-[rgb(var(--fg))] transition"
              >
                View all →
              </Link>
            </div>
              ...
              {featuredPastEvents.map((e) => (
                ...
                >
                  <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.18em] text-[rgb(var(--fg))]/55">
                    <span className="rounded-full border border-black/10 bg-[rgb(var(--bg))] px-3 py-1">
                      {e.category}
                    </span>
                    <span className="truncate">• {e.location}</span>
                  </div>

                  <div className="mt-4 text-base font-semibold tracking-tight text-[rgb(var(--fg))]">
                    {e.title}
                  </div>

                  {/* IMAGE (replaces placeholder gradient) */}
